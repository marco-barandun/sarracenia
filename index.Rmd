...
```{r}
datatable(sarracenia_df %>% select(scientificName, subspecies, county, state, source) %>% filter(source != "3_distribution") %>% filter(!is.na(county)))
```
```{r sarracenia function, include = FALSE, out.width = '50%'}
...
```{r sarracenia map}
map_sarracenia(species = "Sarracenia flava",
